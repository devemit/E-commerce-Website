...
const Product = ({ title, price, description, category, image, product }) => {
  ...
  return (
    <div className='my-20 bg-slate-200 text-center cursor-pointer rounded-t-3xl shadow-2xl m-6 duration-300 hover:-translate-y-2'>
      <div className='bg-gray-900 rounded-t-3xl'>
        <h2 className='text-gray-100 uppercase py-2'>{category}</h2>
        <h1 className='py-2 text-gray-100'>{truncateStr(title, 35)}</h1>
        <p className='text-gray-400 text-sm py-2'>{truncateStr(description, 50)}</p>
      </div>
      <div className='flex items-center justify-center p-6'>
        <img src={image} className='w-48 h-48 mix-blend-multiply' />
      </div>
      <div className='flex justify-between py-4 px-6 bg-gray-900'>
